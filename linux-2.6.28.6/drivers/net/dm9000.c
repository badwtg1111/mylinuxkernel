...
/* add by liuqian  start */
static char mac_addr_uboot[20] = "";
static int __init param_mac_setup(char *str)
{
	strncpy(mac_addr_uboot, str, sizeof(mac_addr_uboot));
}
__setup("mac=", param_mac_setup);

int trans( char *src, unsigned char *k )
{
		char c;
		int i;
		int temp;
...
dm9000_probe(struct platform_device *pdev)
{
	...
	//memcpy(ndev->dev_addr, "\x00\x0C\x29\x34\xE1\x0D", 6);
	/* add by liuqian start */
	trans(mac_addr_uboot, ndev->dev_addr);
	/* add bu liuqian end */
/***************************************************/
