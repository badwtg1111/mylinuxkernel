...
/*******************************************************************************
 * add by suxiaozhi 201111116
 * Modify by xiashengsheng 20120906
 *******************************************************************************/
#if 1
...
#define HdPhone_EintPin  		IRQ_EINT_GROUP(7, 7)
#define HdPhone_GpioPin   		S3C64XX_GPO(7)
//:1ch
//#define OUTPUT_VALUE_HP      	0x00a0				//4~7bit  	10:		HPMIXL/HPMIXR
//no mono output
#define OUTPUT_VALUE_HP      	0x00a0				//4~7bit  	10:		HPMIXL/HPMIXR  --add by liuqian
//#define OUTPUT_VALUE_SPK 		0x1b00				//8~13bit	011: 	SPKMIX/SPKMIX   
//#define OUTPUT_VALUE_SPK 		0x9b00				//8~13bit	011: 	SPKMIX/SPKMIX  --add by liuqian 
//by Johnny playback just enable mono													//8~13bit	010: 	HPMIXL/HPMIXR
#define OUTPUT_VALUE_SPK 		0x8000				//8~13bit	011: 	SPKMIX/SPKMIX  --add by liuqian 
													//8~13bit	010: 	HPMIXL/HPMIXR
#if 0
static irqreturn_t s3c6400_9713HdSp_irq(int irq, void *dev_id)
{
	...
	printk(KERN_INFO "\n In func request_irq(s3c6400_9713HdSp_irq)!!!\n");
	writel( readl( S3C64XX_EINT78MASK ) | ( 1 << 7 ), S3C64XX_EINT78MASK ); 	//disable EINT7 Interrupt before deal it
	writel(	(1 << 7), S3C64XX_EINT78PEND );                               		//disable Interrup

	if(readl(S3C64XX_GPODAT) & (1 << 7))         	                            //如果是耳机，否则是喇叭
    {
		
		printk(KERN_INFO "funciton=%s,line=%d\n",__FUNCTION__, __LINE__);
		ac97_write(codec, AC97_REC_GAIN, OUTPUT_VALUE_HP);
			
		/*disable speaker*/
		tmp  = readl(S3C64XX_GPODAT);
		tmp &= ~(0x1 << 6 );
		writel(tmp, S3C64XX_GPODAT);
	}
    else
	{
		printk(KERN_INFO "funciton=%s,line=%d\n",__FUNCTION__, __LINE__);
	...
	writel(readl(S3C64XX_EINT78MASK) & ( ~(1 << 7) ), S3C64XX_EINT78MASK); 		//Enable Interrupt after deal it
 ...
 *  end Modify by xiashengsheng 20120906
 ****************************************************************************************************************/


/*******************************************************************************
 * add by liuqian 20121206
 *******************************************************************************/
#if 1
static void wm9713_work(struct work_struct *work)
{
	   ...
	   if(readl(S3C64XX_GPODAT) & (1 << 7)) 									   //如果是耳机，否则是喇叭
	   {
		   ac97_write(codec, AC97_REC_GAIN, OUTPUT_VALUE_HP|OUTPUT_VALUE_SPK);
	   ...
	   writel(readl(S3C64XX_EINT78MASK) & ( ~(1 << 7) ), S3C64XX_EINT78MASK);	   //Enable Interrupt after deal it
      enable_irq(HdPhone_EintPin);   //һ������������ʹ���Ǹ�GPIO�ж�
}
...
static irqreturn_t s3c6410_wm9713_HPDET_irq(int irq, void *handle)
{
        ...
        schedule_work(&socdev->work);  //�жϷ����󣬿�ʼ�������
        return IRQ_HANDLED;
}

#endif
/*****************************************************************************************************************
 *	end of add by liuqian
 ****************************************************************************************************************/
...
static int wm9713_soc_probe(struct platform_device *pdev)
{
	...
	/***  add by liuqian 20121206     ***/
	INIT_WORK(&socdev->work, wm9713_work);
	/***  endof add by liuqian        ***/
 ...
 *******************************************************************************/
#if 1
	//add GP06 for speaker select and GPK12 AC97_SPKL/R select
	gpio_direction_output(S3C64XX_GPO(6), 0);
	gpio_direction_output(S3C64XX_GPK(12), 0);

#if 0//Like the above functions -----gpio_direction_output()
	
	u16	 tmp1;
	u32	 tmp2;
	
	/*set GPO6 0001 (output)*/
	tmp2 = readl(S3C64XX_GPOCON);
	tmp2 = (tmp2 & ~(0x3 << 12))|(0x1 << 12);
	writel(tmp2, S3C64XX_GPOCON);
	
	/*disable speaker*/
	tmp1  = readl(S3C64XX_GPODAT);
	tmp1 &= ~(0x1 << 6 );
	writel(tmp1, S3C64XX_GPODAT);


	/* set GPK12 0001 (output)*/
	tmp2 = readl(S3C64XX_GPKCON1);
	tmp2 = (tmp2 & ~(0xf << 16))|(0x1 << 16);
	writel(tmp2, S3C64XX_GPKCON1);

	/*enable AC97_SPKL/R*/
	tmp1  = readl(S3C64XX_GPKDAT);
	tmp1 &= ~(0x1 << 12);
	writel(tmp1, S3C64XX_GPKDAT);
	
#endif
	
	//add headphone interrupt used GPO7
	s3c_gpio_cfgpin(HdPhone_GpioPin, S3C_GPIO_SFN(3));        	//配置GPO7为外中断
	s3c_gpio_setpull(HdPhone_GpioPin, S3C_GPIO_PULL_NONE);  	//不上拉也不下拉，硬件默认拉高
	set_irq_type(HdPhone_EintPin, IRQ_TYPE_EDGE_BOTH);			//插入跟拔出耳机都会触发

	if(readl(S3C64XX_GPODAT) & (1 << 7))              			//如果上电默认是耳机，设置通道；否则设置喇叭
	{
		ac97_write(codec, AC97_REC_GAIN, OUTPUT_VALUE_HP);
	}
	else
	...
	//给IRQ_EINT_GROUP注册中断和中断处理函数，参数包括中断方式,中断名
	//ret = request_irq(HdPhone_EintPin, s3c6400_9713HdSp_irq, IRQF_DISABLED, "HdphonSpeak", NULL);
	ret = request_irq(HdPhone_EintPin, s3c6410_wm9713_HPDET_irq, IRQF_DISABLED, "HdphonSpeak", socdev);
	if(ret < 0)
	{
		printk(KERN_ERR "request_irq(s3c6400_9713HdSp_irq) failed !!!\n");

		goto irq_err;
	}
	else if(ret == 0)
	{
		printk(KERN_ERR "request_irq(s3c6400_9713HdSp_irq) successed !!!\n");
	}

#endif
/*****************************************************************************************************
 *	endof add by suxiaozhi
 * ***************************************************************************************************/
